<footer id="about" class="page-footer elegant-color center-on-small-only">
    <div class="container-fluid">
        <div class="row">

            <!--First column-->
            <div class="col-md-4 text-center">
                <div class="column-content">
                    <div class="column-title">
                      <img class="responsive" src="img/italia.png">  
                    </div>
                </div>
            </div>
            <!--/.First column-->

            <!--Second column-->
            <div class="col-md-4 text-center">
                <p class="column-title white-text">ITTS "O. Belluzzi – L. da Vinci"</p>
                <div class="column-content">
                    <p class="thin-100 white-text break-word">Via Ada Negri n. 34 - 47923 Rimini
                    <br>
                    0541/384211
                    <br>
                    RNTF010004@pec.istruzione.it
                    <br>
                    RNTF010004 - Cod.Fisc. 82007870403
                    <br>
                </p>
                </div>
            </div>
            <!--/.Second column-->

            <!--Third column-->
            <div class="col-md-4 text-center">
                <p class="column-title white-text">ITTS "O. Belluzzi – L. da Vinci"</p>
                <div class="column-content">
                    <p class="thin-100 white-text">Il sito realizzato da Stanislav
                </p>
            </div>
            <!--/.Third column-->

        </div>
    </div>

    <div class="social-section text-center">
        <a href="https://www.facebook.com/ittsrimini" target="_blank" class="btn-sm-full fb-bg rectangle waves-effect waves-light"><i class="fa fa-facebook"> </i> <span>Facebook</span> </a>
        <a class="btn-sm-full tw-bg rectangle waves-effect waves-light"><i class="fa fa-twitter"> </i> <span>Twitter</span></a>
        <a href="https://www.youtube.com/user/ItisRimini" target="_blank" class="btn-sm-full youtube-bg rectangle waves-effect waves-light"><i class="fa fa-youtube-play"> </i> <span>YouTube</span></a>
    </div>


    <div class="row">
        <div class="footer-copyright text-center rgba-black-light">
            <div class="container-fluid">
                © 2016 Copyright
            </div>
        </div>
    </div>
    
</footer>